    ...
    public class Worker : BackgroundService
    {
        ...
        private async Task Consumer_Received(object sender, BasicDeliverEventArgs @event)
        {
           ...
           var createExcelMessage = JsonSerializer.Deserialize<CreateExcelMessage>(Encoding.UTF8.GetString(@event.Body.ToArray()));
            ...
            multipartFormDataContent.Add(new ByteArrayContent(ms.ToArray()), "file", Guid.NewGuid().ToString() + ".xslx");
            ...
            using (var httpClient = new HttpClient())
            {
                ...
                {
                    _logger.LogInformation($"File ( Id : {createExcelMessage.FileId}) was created bu seccesful");
        ...
        private DataTable GetTable(string tableName)
        {
            ...
            table.Columns.Add("EnglishProductName", typeof(string));
            table.Columns.Add("ProductAlternateKey", typeof(string));
            table.Columns.Add("Color", typeof(string));

            products.ForEach(x =>
            {
                table.Rows.Add(x.ProductKey,x.EnglishProductName,x.ProductAlternateKey,x.Color);
            });
